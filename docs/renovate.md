...
- Manages only the root `package.json` dependencies. The template `template/package.json` is excluded from Renovate entirely by a file-level package rule (`matchFileNames: ["template/package.json"], enabled: false`), so Renovate never proposes template updates on GitHub. The template is maintained locally by the maintainer using the `pnpm template:update-lock` script, which regenerates `template/pnpm-lock.yaml` by substituting the template placeholders and running `pnpm install --ignore-scripts`.
