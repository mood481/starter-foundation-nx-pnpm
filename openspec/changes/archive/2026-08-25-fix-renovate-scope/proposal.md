...
1. **The OpenSpec-scope workflow only rejects drift; it never fixes it.** On the Mend Renovate GitHub App, `postUpgradeTasks` are ignored unless allowlisted, so an OpenSpec update PR arrives with the dependency bumped but the checked-in assistant tooling (`.agents/skills/`, `.opencode/commands/`, `.opencode/skills/`) stale. The workflow then fails the drift gate and blocks `platformAutomerge`. The workflow should instead regenerate and commit the tooling on Renovate PRs, so the gate passes and automerge can proceed without a human in the loop.
