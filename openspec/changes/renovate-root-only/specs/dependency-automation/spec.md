...
- ### Requirement: OpenSpec Tooling Regeneration on Dependency Update
  ...
  **Reason**: Renovate's `postUpgradeTasks` are blocked by the hosted Renovate app's `allowedPostUpgradeCommands` (only `git add --all`, `git reset`, `pwd` are permitted), so the OpenSpec-scope workflow now regenerates the assistant tooling on `renovate/*` branches instead. `renovate.json` therefore omits `postUpgradeTasks`.

  **Migration**: Covered by the `OpenSpec Scope Pull Request Gate` requirement — the workflow configures the OpenSpec profile, delivery, and workflows and runs `ospec:update`, then commits `.opencode/commands` and `.opencode/skills` on `renovate/*` branches.
