...
- [pnpm package file expansion] A local `template/node_modules/` can be included by pnpm's broad `template/**` files pattern. → D8 and the package file-set validation explicitly exclude nested dependency directories.
